...
then
    for lang in "${LANGS[@]}"; do
        ...
        *) model="" ;;
        ...
        if [ -n "$model" ]; then
            echo "Downloading MFA acoustic + dictionary for $lang ($model)..."
            .venv/bin/mfa model download acoustic "$model" || echo "  ⚠ acoustic $model failed"
            .venv/bin/mfa model download dictionary "$model" || echo "  ⚠ dictionary $model failed"
        fi
    done
else
    echo "⚠ MFA unavailable: kalpy/_kalpy is missing."
    echo "  SpeechPrint will still run ASR, prosody, corpus creation, and exports."
    echo "  Forced alignment is disabled until MFA/kalpy is installed separately."
fi
