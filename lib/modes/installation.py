...
class ConfirmationStep:
    ...
    def build_ui(self, container):
        ...
        info = Gtk.Label()
        info.set_markup("""Configures this machine for SpeechPrint analysis:
...
class DependenciesStep:
    ...
    async def execute(self):
        # script_dir is passed in from InstallationMode (which reads cfg.scripts_dir);
        # also accept a fallback to the canonical lib/scripts/ next to this module.
        script = Path(self.script_dir) / "install_deps.sh"
        if not script.exists():
            fallback = Path(__file__).resolve().parent.parent / "scripts" / "install_deps.sh"
            if fallback.exists():
                script = fallback
        if not script.exists():
            self._log(f"✗ install_deps.sh not found at {script}")
            self._log(f"  Also tried: {Path(__file__).resolve().parent.parent / 'scripts' / 'install_deps.sh'}")
