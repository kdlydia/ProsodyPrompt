...
underlying fundamental frequency (F0) measurements. ProsodyPrompt originally used
...
narrower-range recordings. ProsodyPrompt v3 therefore auto-detects the speaker's
...
ProsodyPrompt is implemented as a Python pipeline organized into nine sequential
...
In ProsodyPrompt v3, intensity is extracted using Parselmouth with a 40\,ms
...
The evaluation of ProsodyPrompt's prosody output takes two forms.
...
\item ProsodyPrompt's prosody labels are assigned using the procedure described
      in Section~\ref{sec:prosody_label_method}.
\item The GToBI tone symbol and its position within the word sequence are
      compared to the ProsodyPrompt label on the corresponding syllable.
\end{enumerate}
...
A design principle of ProsodyPrompt is that the prosody representation should be
...
sequence: (1) a raw audio recording is shown in Praat; (2) ProsodyPrompt
...
ProsodyPrompt word-alignment and phonemization stages (Stages 2--4 and 6) are
...
glosses), \texttt{ph@6} (SAMPA phones). The ProsodyPrompt pipeline was
