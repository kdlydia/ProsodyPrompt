\chapter{SpeechPrint: System Design and Implementation}
\label{speechprint}

This chapter describes the SpeechPrint system: its core methods (pitch extraction, alignment, and prosody labeling), the system architecture, and the evaluation design.

\section{System Overview and Pipeline Architecture}

\subsection{System Overview}

SpeechPrint takes a WAV recording and produces a Praat TextGrid with five time-aligned tiers: words, phonemes, syllables, fundamental frequency at vowel nuclei, and symbolic prosody labels. The system runs in nine sequential stages.

\subsection{The Nine Processing Stages}

\paragraph{Stage 1: Load audio.}
Read the WAV file to get sample rate, number of channels, and duration.

\paragraph{Stage 2: Transcribe.}
Use WhisperX to get a transcript and word timestamps.

\paragraph{Stage 3: Clean transcript.}
Remove punctuation. Convert to lowercase. Mark pauses as \texttt{<sil>}.

\paragraph{Stage 4: Forced phoneme alignment.}
For English: use Montreal Forced Aligner (MFA) for exact phoneme timing. For other languages: use WhisperX alignment or distribute phonemes proportionally.

\paragraph{Stage 5: Extract pitch and loudness.}
Measure pitch (F0) with pYIN. Measure loudness with Parselmouth. Store both for later lookup.

\paragraph{Stage 6: Phonemize and syllabify.}
Convert words to IPA phonemes. Find syllable boundaries by splitting consonant clusters between vowels.

\paragraph{Stage 7: Assign prosody labels.}
Measure pitch rise/fall in each syllable. Compare to neighbors. Assign symbols: \texttt{/} rising, \texttt{\textbackslash} falling, \texttt{\={}} high, \texttt{\_} low, \texttt{*} prominent.

\paragraph{Stage 8: Write TextGrid.}
Write the five tiers as a Praat TextGrid file.

\paragraph{Stage 9: Export tables.}
Write CSV files with the data for analysis.

\section{Methods and Implementation}

\subsection{Pitch Extraction and Octave Error Correction}
...
Praat's default autocorrelation (AC) pitch tracker, which is
...
\item \textbf{Praat AC (raw):} The default Praat pitch tracker, using the
      autocorrelation (AC) method with a pitch floor of 75\,Hz and ceiling of
...
\item \textbf{Librosa pYIN (probabilistic YIN):} A state-of-the-art monophonic
      ...
      samples and a hop length of 256 samples, giving a temporal resolution of
      256/$f_s$ seconds (approximately 16\,ms for audio sampled at 16\,kHz, though the
      implementation applies this hop length to the original audio sample rate).
...
\item \textbf{CREPE:} The Convolutional REpresentation for Pitch Estimation
      ...
      CREPE requires audio to be resampled to 16\,kHz before inference; this
      resampling is performed as a preprocessing step using librosa before
      calling \texttt{torchcrepe.predict()}.
...
\subsection{Technical Implementation and Architecture}
\label{sec:implementation}

This section details the technical realization of SpeechPrint's 9-stage pipeline.
...
\subsection{Syllabification and Nucleus Identification}
...
\subsection{Prosody Labelling Algorithm}
...
The weak-rise threshold $\theta_{\mathrm{weak}}$ is computed from the normalised median absolute deviation (MAD) of all intra-syllable pitch movements in the recording:
\begin{equation}
  \theta_{\mathrm{weak}} = \max\!\left(\theta_{\mathrm{floor}},\;
    \alpha \cdot \mathrm{MAD}(\{\Delta_{F_0,i}\})\right)
\end{equation}
with a floor of $\theta_{\mathrm{floor}} = 0.5$\,ST and an adaptive factor of
$\alpha = 0.35$. The normalised MAD is computed as $\text{MAD} \times 1.4826$, making it a robust estimate of scale that is insensitive to outliers. A strong movement is defined as
...
\subsection{Amplitude Analysis: Window Size and Relative Measurement}
\label{sec:amplitude}
...
This section describes how SpeechPrint's prosody output was evaluated.

\subsection{Benchmark 1: Automatic GToBI comparison}
...
\subsection{Benchmark 2: Perceptual evaluation via questionnaire}
...
\subsection{Symbol Design and Demo Visualization}
...
\subsection{Linguistic Rationale for Syllable-Level Annotation}
...
\section{Applications to Endangered Languages: DoReCo Corpus}
\label{sec:doreco_pipeline}

\subsection{Daakie (Vanuatu): Human-Annotated Pipeline}
...
\subsection{Cabécar (Costa Rica): Cross-Linguistic Validation}
...
\subsection{Example Pipeline Output: Phoneme-Level Measurements}

The following table illustrates the merged output from the forced aligner (MFA) combined with acoustic measurements (Parselmouth F0 and amplitude extraction). This demonstrates the structured data produced by the pipeline for downstream analysis:

\begin{table}[h]
\centering
\caption{Merged MFA phoneme timings with Parselmouth F0/amplitude extraction}
