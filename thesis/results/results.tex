...
quantifiable. ProsodyPrompt's symbolic layer computes the intra-syllable pitch
movement as:
%
\begin{equation}
...
pattern visible in the ProsodyPrompt v1 and v2 outputs where the supervisor
...
resulting prosody labels in ProsodyPrompt v3 show qualitatively more consistent
...
be compared directly with ProsodyPrompt prosody labels. Human word boundaries from
...
Three sets of results are reported: (1)~ProsodyPrompt v3 (pYIN, baseline pipeline,
...
\subsection{ProsodyPrompt v3 — pYIN baseline}

Table~\ref{tab:gtobi_pyin} shows results for the pYIN-based pipeline.

\begin{table}[h]
\centering
\caption{GToBI vs.\ ProsodyPrompt v3 (pYIN) prosody labels. Syllables in IPA;
...
(MFA) used in earlier ProsodyPrompt versions had produced wrong transcriptions for
...
\begin{table}[h]
\centering
\caption{GToBI vs.\ ProsodyPrompt BEST (CREPE + 5 optimisations, corrected IPA).
...
\begin{figure}[htbp]
  ...
  \caption{ProsodyPrompt BEST TextGrid output for all five German GToBI sentences
...
\begin{table}[h]
\centering
\caption{GToBI vs.\ ProsodyPrompt final pipeline (pYIN + 5 optimisations, corrected IPA).
...
ProsodyPrompt v3 uses the local neighbourhood comparison (Section~\ref{sec:prosody_label_method})
