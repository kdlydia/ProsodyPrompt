...
# Adaptive prosody thresholds. Floors here, but the actual cutoff is derived
# per recording from the std deviation of pitch movements (see _label_prosody).
F0_RISE_FLOOR_ST = 3.0    # Minimum semitones to call a rise (was 1.5 — too sensitive)
F0_FALL_FLOOR_ST = 3.0    # Minimum semitones to call a fall
F0_STD_FACTOR = 0.75      # Movement must also exceed this × std_dev of all movements
...
def label_prosody(syllables: list[dict]) -> None:
    """Measurement-based symbolic prosody labels.
    ...
    Symbols:
      ...
      –  high level F0
      _  low level F0
      *  locally prominent syllable
    """
    if not syllables:
        return

    f0_vals = [x for x in (sy.get("mean_f0_hz") for sy in syllables) if x]
    int_vals = [x for x in (sy.get("mean_intensity_db") for sy in syllables) if x is not None]
    dur_vals = [max(0.001, float(sy["end"] - sy["start"])) for sy in syllables]
    mean_f0 = _safe_mean(f0_vals) if f0_vals else None
    ...
    def zscore(x, vals):
        ...
        if sd <= 1e-9:
            return 0.0
        return (float(x) - _safe_mean(vals)) / sd

    # First pass: ProsodyPro-like interval measurements.
    movements = []
    excursions = []
    velocities = []

    for sy in syllables:
        dur_s = max(0.001, float(sy["end"] - sy["start"]))

        onset = sy.get("onset_f0_hz") or sy.get("mean_f0_hz")
        offset = sy.get("offset_f0_hz") or sy.get("mean_f0_hz")
        mn = sy.get("min_f0_hz") or sy.get("mean_f0_hz")
        mx = sy.get("max_f0_hz") or sy.get("mean_f0_hz")

        mv = _semitone(onset, offset) if (onset and offset) else None
        exc = _semitone(mn, mx) if (mn and mx) else None
        vel = (mv / dur_s) if mv is not None else None

        sy["pitch_movement_st"] = mv
        sy["duration_ms"] = dur_s * 1000.0
        sy["excursion_size_semitones"] = exc
        sy["f0_velocity_st_per_s"] = vel
        sy["final_f0_hz"] = offset
        sy["relative_height_st"] = _semitone(mean_f0, sy.get("mean_f0_hz")) if (mean_f0 and sy.get("mean_f0_hz")) else None
        sy["norm_f0_z"] = zscore(sy.get("mean_f0_hz"), f0_vals)
        sy["norm_intensity_z"] = zscore(sy.get("mean_intensity_db"), int_vals)
        sy["norm_duration_z"] = zscore(dur_s, dur_vals)
    ...
    rise_threshold = max(0.45, min(1.25, F0_RISE_FLOOR_ST + 0.35 * std_mv))
    fall_threshold = -max(0.45, min(1.25, F0_FALL_FLOOR_ST + 0.35 * std_mv))

    # Second pass: symbolic interpretation.
    for idx, sy in enumerate(syllables):
        mv = sy.get("pitch_movement_st")
        height = sy.get("relative_height_st") or 0.0

        if mv is None:
            sy["pitch_movement"] = "unknown"
            sy["symbol"] = "?"
        elif mv >= rise_threshold:
            sy["pitch_movement"] = "rising"
            sy["symbol"] = "/"
        elif mv <= fall_threshold:
            sy["pitch_movement"] = "falling"
            sy["symbol"] = chr(92)
        else:
            sy["pitch_movement"] = "high_level" if height >= 0 else "low_level"
            sy["symbol"] = "–" if height >= 0 else "_"

        int_z = sy.get("norm_intensity_z", 0.0)
        dur_z = sy.get("norm_duration_z", 0.0)
        f0_z = abs(sy.get("norm_f0_z", 0.0))
        exc_z = abs(zscore(sy.get("excursion_size_semitones"), excursions))
        vel_z = abs(zscore(sy.get("f0_velocity_st_per_s"), velocities))

        local_amp = False
        cur_int = sy.get("mean_intensity_db")
        if cur_int is not None:
            neigh = []
            if idx > 0 and syllables[idx - 1].get("mean_intensity_db") is not None:
                neigh.append(syllables[idx - 1]["mean_intensity_db"])
            if idx + 1 < len(syllables) and syllables[idx + 1].get("mean_intensity_db") is not None:
                neigh.append(syllables[idx + 1]["mean_intensity_db"])
            local_amp = bool(neigh and cur_int - max(neigh) >= 1.5)

        # Measurement-weighted prominence.
        sy["prominence_score"] = (
            1.50 * int_z +
            1.20 * dur_z +
            0.85 * f0_z +
        ...
        # Mild prefinal preference, but only as a tie-breaker.
        if 0 < idx < len(syllables) - 1:
            sy["prominence_score"] += 0.35
        if idx == len(syllables) - 1:
            sy["prominence_score"] -= 0.60

        sy["is_prominent"] = local_amp

    # Ensure one main star. Prefer prefinal if it is close to strongest.
    if not any(sy.get("is_prominent") for sy in syllables):
        best = max(range(len(syllables)), key=lambda i: syllables[i].get("prominence_score", 0.0))
        if len(syllables) >= 3:
            prefinal = len(syllables) - 2
            if syllables[prefinal].get("prominence_score", 0.0) >= syllables[best].get("prominence_score", 0.0) - 0.75:
                best = prefinal
        syllables[best]["is_prominent"] = True

    for sy in syllables:
        base = sy.get("symbol", "?")
        sy["symbol_marked"] = base + "*" if sy.get("is_prominent") and base != "?" else base
        score = sy.get("prominence_score", 0.0)
        sy["stress_class"] = "SS3" if sy.get("is_prominent") else ("SS2" if score >= 1.0 else "SS1")
...
def write_textgrid(
    ...
    tiers = []
    if extra_word_tiers:
        tiers.extend(extra_word_tiers)
    tiers.extend([
        ("words_selected", word_rows),
        ("syllables", syll_rows),
        ("phonemes", phone_rows),
        ("f0_pitch", f0_rows),
...
# ============================================================================
# MFA-AWARE SYLLABIFICATION (used when MFA produced real phone intervals)
# ============================================================================


# IPA + ARPABET vowel detection. MFA's phones come back in either form
# depending on the dictionary; this set covers both.
_MFA_VOWEL_CHARS = set("aeiouæɑɒɐəɚɛɜɝɪɨɔøœuʊʌyɯɵɘɤɞãĩõũ")
_MFA_ARPABET_VOWELS = {
...
# ARPABET consonants that overlap with IPA vowel characters when lowercased
# (so we can short-circuit the IPA check). E.g. ARPABET "Y" is a consonant
# glide but lowercases to "y" which IS an IPA vowel.
_MFA_ARPABET_CONSONANTS = {
...
def _is_vowel_phone(label: str) -> bool:
    ...
    if not raw:
        return False
    # ARPABET form: strip trailing stress digit (AH0, IY1, …)
    bare = re.sub(r"\d", "", raw).upper()
    if bare in _MFA_ARPABET_VOWELS:
        return True
    if bare in _MFA_ARPABET_CONSONANTS:
        return False
    # IPA form: any vowel character in the label
    return any(c in _MFA_VOWEL_CHARS for c in raw.lower())


def _phone_label_to_ipa(label: str) -> str:
    """Best-effort: pass IPA through unchanged, map ARPABET to IPA."""
    arpa = re.sub(r"\d", "", label.strip()).upper()
    if arpa in _MFA_VOWEL_CHARS:  # already IPA
        return label.strip()
    arpa_to_ipa = {
        "AA": "ɑ", "AE": "æ", "AH": "ʌ", "AO": "ɔ", "AW": "aʊ", "AY": "aɪ",
        "EH": "ɛ", "ER": "ɝ", "EY": "eɪ", "IH": "ɪ", "IY": "i", "OW": "oʊ",
        "OY": "ɔɪ", "UH": "ʊ", "UW": "u", "SH": "ʃ", "ZH": "ʒ", "CH": "tʃ",
        "JH": "dʒ", "TH": "θ", "DH": "ð", "NG": "ŋ", "Y": "j",
    }
    return arpa_to_ipa.get(arpa, label.strip().lower())


def _phones_for_word(phones: list[dict], w_start: float, w_end: float) -> list[dict]:
    """Phone intervals whose midpoint falls inside [w_start, w_end]."""
    out = []
    for p in phones:
        mid = (p["start"] + p["end"]) / 2
...
def _group_phones_into_syllables(word_phones: list[dict]) -> list[list[dict]]:
    """Maximal-onset-style grouping anchored on vowel nuclei.

    Walks the phone sequence and produces a *disjoint* partition: every
    phone belongs to exactly one syllable.

    Strategy:
      - Find all vowel-nucleus indices.
      - Inter-vocalic consonant runs are split: the second half attaches
        to the FOLLOWING syllable as onset (maximal-onset preference);
        the first half stays as coda of the preceding syllable. An odd
        leftover consonant goes to the next syllable's onset.
      - Phones before the first nucleus are onset of syllable 1.
      - Phones after the last nucleus are coda of the last syllable.
    """
    if not word_phones:
        return []

    nuclei = [i for i, p in enumerate(word_phones) if _is_vowel_phone(p["phone"])]
    if not nuclei:
        return [list(word_phones)]

    # Decide the cut index between each consecutive pair of nuclei.
    # cuts[k] = first index that belongs to syllable k+1 (i.e. the boundary
    # AFTER nucleus k). cuts[-1] is len(word_phones) (end of last syllable).
    cuts: list[int] = []
    for k in range(len(nuclei) - 1):
        a = nuclei[k] + 1            # first inter-vocalic consonant index
        b = nuclei[k + 1]            # next nucleus index (exclusive end of inter-V run)
        run = b - a                  # number of inter-vocalic consonants
        # Maximal onset: floor(run/2) become coda of left syllable,
        # ceil(run/2) become onset of right syllable.
        coda_count = run // 2
        cuts.append(a + coda_count)
    cuts.append(len(word_phones))    # end of the last syllable

    groups: list[list[dict]] = []
    start = 0
    for end in cuts:
        groups.append(word_phones[start:end])
        start = end
    return groups


def build_syllables_from_mfa(word_intervals: list[dict],
                              mfa_phones: list[dict]) -> tuple[list[dict], list[dict]]:
    """Use MFA's real phone boundaries to build syllables + phoneme rows.

    Returns (syllables, phonemes) in the same shape as the proportional
    builder so the rest of the pipeline doesn't have to care. Each
    syllable's `phones` field is a space-separated IPA string; each
    phoneme row has exact MFA timing.
    """
    syllables: list[dict] = []
    phonemes: list[dict] = []
    syll_idx = 0
    phone_idx = 0
    for wi, w in enumerate(word_intervals):
        w_phones = _phones_for_word(mfa_phones, w["start"], w["end"])
        if not w_phones:
            # No MFA phones in this word's span — fall back to a single empty
            # syllable so the indexing stays sane downstream
            syll_idx += 1
            syllables.append({
                "index": syll_idx, "label": w["word"], "word_index": wi + 1,
            ...
            continue
        groups = _group_phones_into_syllables(w_phones)
        for g in groups:
            if not g:
                continue
            syll_idx += 1
            s_start = g[0]["start"]
            s_end = g[-1]["end"]
            syl_label = "".join(_phone_label_to_ipa(p["phone"]) for p in g)
            syllables.append({
                "index": syll_idx, "label": syl_label, "word_index": wi + 1,
                "start": s_start, "end": s_end,
                "mid": (s_start + s_end) / 2,
                "duration_s": s_end - s_start,
                "phones": " ".join(_phone_label_to_ipa(p["phone"]) for p in g),
            })
            for ph in g:
                phone_idx += 1
                phonemes.append({
                    "index": phone_idx,
                    "label": _phone_label_to_ipa(ph["phone"]),
                    "syllable_index": syll_idx,
                    "word_index": wi + 1,
                    "start": ph["start"], "end": ph["end"],
                    "mid": (ph["start"] + ph["end"]) / 2,
                    "duration_s": ph["end"] - ph["start"],
                })
...
def annotate(args) -> int:
    ...
    aligner_name = getattr(args, "aligner", "whisperx") or "whisperx"
    compare = bool(getattr(args, "compare_aligners", False))

    # Multi-aligner path: run every requested backend, write per-aligner
    # TextGrid + a comparison CSV. The "primary" aligner whose output
    # populates the main 6-tier TextGrid is still `args.aligner`.
    comparison_results = []  # filled if compare=True
    if compare:
        try:
            from speechprint_pkg import aligners as _alg
            print("  Running aligner comparison …", flush=True)

            selected_aligner_names = None
            raw_aligners = getattr(args, "aligners", None)
            if raw_aligners:
                selected_aligner_names = [
                    x.strip() for x in raw_aligners.split(",") if x.strip()
                ]

            comparison_results = _alg.run_all(
                wav, transcript["text"], args.language,
                include=selected_aligner_names,
            ...
            for r in comparison_results:
                ...
                print(
                    f"    [{r['aligner']:>15s}] "
                    f"{'OK' if r['available'] else 'unavailable':>11s}  "
                    f"words={len(r['words'])} "
            ...
            warnings.append(f"Aligner comparison failed: {e}")

    word_intervals: list[dict] = []
    align_warns: list[str] = []
    used_aligner = aligner_name
    mfa_phones: list[dict] | None = None  # set when MFA ran and produced phone intervals

    if compare and comparison_results:
        # Prefer the user-chosen aligner's words; fall back to first available
        primary = next((r for r in comparison_results
                        if r["aligner"] == aligner_name and r["available"]), None)
        if primary is None:
            primary = next((r for r in comparison_results if r["available"]), None)
        if primary is not None:
            word_intervals = primary["words"]
            used_aligner = primary["aligner"]
            align_warns = list(primary["warnings"])
            if used_aligner != aligner_name:
                align_warns.append(
                    f"Requested aligner '{aligner_name}' was unavailable; "
                    f"using '{used_aligner}' instead."
                )
        else:
            align_warns.append("All requested aligners were unavailable")
        # Whenever MFA was part of the comparison and produced phones,
        # keep them — we'll use them for the syllable layer regardless of
        # which aligner was 'primary'.
        mfa_run = next((r for r in comparison_results
                        if r["aligner"] == "mfa" and r["available"] and r.get("phones")), None)
        if mfa_run:
            mfa_phones = mfa_run["phones"]

    # Single-aligner path (or compare gave us nothing): dispatch directly
    if not word_intervals:
        if aligner_name == "whisperx":
            word_intervals, align_warns = align_words(wav, transcript, args.language)
            used_aligner = "whisperx"
        else:
            try:
                from speechprint_pkg import aligners as _alg
                fn = _alg.ALIGNER_FNS.get(aligner_name)
                if fn is None:
                    raise RuntimeError(f"Unknown aligner: {aligner_name}")
                if aligner_name == "whisperx":
                    res = fn(wav, transcript["text"], args.language,
                             whisper_segments=transcript.get("segments"))
                else:
                    res = fn(wav, transcript["text"], args.language)
                if res["available"]:
                    word_intervals = res["words"]
                    used_aligner = aligner_name
                    align_warns = list(res["warnings"])
                    if aligner_name == "mfa" and res.get("phones"):
                        mfa_phones = res["phones"]
                else:
                    # Fall back to the original WhisperX-or-degraded path
                    align_warns.extend(res["warnings"])
                    align_warns.append(
                        f"Aligner '{aligner_name}' unavailable; falling back to default path."
                    )
                    wi2, w2 = align_words(wav, transcript, args.language)
                    word_intervals = wi2
                    align_warns.extend(w2)
                    used_aligner = "whisperx"
            except Exception as e:
                align_warns.append(f"Aligner dispatch error: {e}; falling back.")
                word_intervals, fb_warns = align_words(wav, transcript, args.language)
                align_warns.extend(fb_warns)
                used_aligner = "whisperx"

    warnings.extend(align_warns)
    if used_aligner != aligner_name and not compare:
        warnings.append(f"Used aligner: {used_aligner}")
    ...
    if mfa_phones:
        # Real phone timing path: use MFA's millisecond-accurate boundaries.
        # No phonemizer fallback needed; we have ground truth from the aligner.
        syllables, phonemes = build_syllables_from_mfa(word_intervals, mfa_phones)
        print(f"  ✓ Built {len(syllables)} syllables + {len(phonemes)} phones "
              f"from MFA timings", flush=True)
    else:
        # Proportional path (original): phonemizer → uniform distribution
        # inside each syllable. This is the WhisperX / Gentle / CrisperWhisper
        # case where no aligner emitted phone-level timing.
    ...
    master_extra_word_tiers = []
    if comparison_results:
        for _r in comparison_results:
            _an = _r.get("aligner", "unknown")
            _tier = f"words_{_an}"
            if _r.get("available") and _r.get("words"):
                master_extra_word_tiers.append((
                    _tier,
                    [{"start": w["start"], "end": w["end"], "value": w["word"]} for w in _r["words"]]
                ))
            else:
                _msg = "UNAVAILABLE: " + "; ".join(_r.get("warnings") or ["no usable words"])
                master_extra_word_tiers.append((
                    _tier,
                    [{"start": 0.0, "end": min(duration, 0.6) if duration else 0.6, "value": _msg}]
                ))
    ...
    write_textgrid(
        ...
        phonemes,
        warn_text,
        extra_word_tiers=master_extra_word_tiers,
    )
    ...
    manifest = {
        ...
        "tier_set": ["words_selected", "syllables", "phonemes", "f0_pitch", "prosody_labels", "warnings_review"],
    }
    ...
    # If multiple aligners were run, dump per-aligner TextGrids + a comparison CSV
    aligner_outputs = []
    if comparison_results:
        try:
            from speechprint_pkg import aligners as _alg
            aligners_dir = out / "aligners"
            aligners_dir.mkdir(exist_ok=True)
            for r in comparison_results:
                aligner_name = r.get("aligner", "unknown")
                if not r.get("available") or not r.get("words"):
                    fail_words = [{
                        "start": 0.0,
                        "end": min(duration, 0.6) if duration else 0.6,
                        "word": "UNAVAILABLE: " + "; ".join(r.get("warnings") or ["no usable words"])
                    }]
                    tg_path = aligners_dir / f"{name}.{aligner_name}.TextGrid"
                    json_path = aligners_dir / f"{name}.{aligner_name}.json"
                    _alg.write_simple_textgrid(fail_words, duration, tg_path, aligner_name)
                    json_path.write_text(json.dumps(r, indent=2, ensure_ascii=False), encoding="utf-8")
                    aligner_outputs.append({
                    ...
                    continue
                tg_path = aligners_dir / f"{name}.{aligner_name}.TextGrid"
                _alg.write_simple_textgrid(r["words"], duration, tg_path, aligner_name)
                json_path = aligners_dir / f"{name}.{aligner_name}.json"
                ...
                aligner_outputs.append({
                    ...
                    "warnings": r["warnings"],
                    "textgrid": str(tg_path.relative_to(out)),
                    "json": str(json_path.relative_to(out)),
                })
            comp_csv = aligners_dir / "comparison.csv"
            n_rows = _alg.write_comparison_csv(comparison_results, comp_csv)
            try:
                _alg.write_combined_textgrid(
                    comparison_results,
                    aligners_dir / "selected_aligners.TextGrid",
                    duration,
                )
            except Exception as exc:
                warnings.append(f"Combined aligner TextGrid failed: {exc}")
            if n_rows > 0:
                print(f"✓ Wrote aligner comparison ({n_rows} rows): "
                      f"{comp_csv.relative_to(out)}", flush=True)

                try:
                    master_tg = out / f"{name}.master_emet.TextGrid"
                    selected_tg = aligners_dir / "selected_aligners.TextGrid"
                    ...
                    aligner_txt = selected_tg.read_text(encoding="utf-8")
                    canon_txt = (out / f"{name}.TextGrid").read_text(encoding="utf-8")
                    ...
                    def extract_xmax(txt):
                        m = re.search(r"^xmax = ([0-9.eE+-]+)", txt, flags=re.M)
                        return float(m.group(1)) if m else duration

                    items = extract_items(aligner_txt) + extract_items(canon_txt)
                    xmax = max(extract_xmax(aligner_txt), extract_xmax(canon_txt), duration)
                    ...
                    master_tg.write_text("\n".join(lines) + "\n", encoding="utf-8")
                    print(f"✓ Wrote master EmEt TextGrid: {master_tg.relative_to(out)}", flush=True)

                except Exception as exc:
                    warnings.append(f"Master EmEt TextGrid failed: {exc}")
            else:
                print("  (no aligners ran successfully — no comparison CSV written)",
                      flush=True)
        except Exception as e:
            warnings.append(f"Comparison output failed: {e}")

    (out / "run_metadata.json").write_text(json.dumps({
        "backend": "speechprint_pipeline",
        "version": "0.5.0",
        "tiers": ["words_selected", "syllables", "phonemes", "f0_pitch", "prosody_labels", "warnings_review"],
        "stages": STAGES,
        "alignment_source": transcript["source"],
        "primary_aligner": used_aligner,
        "phone_source": "mfa" if mfa_phones else "phonemizer_proportional",
        "compared_aligners": aligner_outputs,
    }, indent=2))
...
def _aligners_cmd(args) -> int:
    """Print which aligner backends are installed/reachable right now."""
    try:
        from speechprint_pkg import aligners as _alg
    except Exception as e:
        print(f"ERROR: could not import aligners module: {e}", file=sys.stderr)
        return 1
    rep = _alg.availability_report()
    print(json.dumps(rep, indent=2))
    n_ok = sum(1 for v in rep.values() if v["installed"])
    print(f"\n{n_ok} of {len(rep)} aligners are installed/reachable.")
    return 0
...
def write_combined_aligner_textgrid(results, out_path):
    from pathlib import Path
    ...
    for name, result in results.items():
        ...
        tiers.append((f"words_{name}", intervals))
    ...
    for i, (tier_name, intervals) in enumerate(tiers, 1):
...
def main() -> int:
    ...
    a.add_argument(
        "--aligner", default="whisperx",
        choices=["whisperx", "mfa", "gentle", "crisperwhisper", "all"],
        help="Which forced aligner to use for the primary 6-tier TextGrid "
             "(default: whisperx)",
    )
    a.add_argument(
        "--compare-aligners", action="store_true",
        help="Also run every available aligner and write per-aligner "
             "TextGrids + a comparison CSV under aligners/",
    )
    a.add_argument(
        "--aligners",
        default=None,
        help="Comma-separated aligners to run in comparison mode, e.g. whisperx,gentle,mfa",
    )
    ...
    al = sub.add_parser(
        "aligners",
        help="Report which aligner backends are installed / reachable",
    )
    al.set_defaults(func=_aligners_cmd)
